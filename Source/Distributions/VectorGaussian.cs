...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	public class VectorGaussian : IDistribution<Vector>, 
		...
		CanGetAverageLog<VectorGaussian>, CanGetLogNormalizer
	{
		DenseVector meanTimesPrecision;
		...
		/// </summary>
		[System.Xml.Serialization.XmlElement(Type=typeof(SerializableMatrix))]
...
#if false
		protected void GetMeanAndVariance(out double mean, out double variance)
		{
			...
			if (IsPointMass) {
				...
				variance = 1.0 / precision[0];
				mean = variance * meanTimesPrecision[0];
			}
		}
		protected void SetMeanAndVariance(double mean, double variance)
		{
			...
			if (variance == 0.0) {
				SetToPointMass();
				meanTimesPrecision[0] = mean;
				return;
			}
			double prec = 1.0 / variance;
			precision[0] = prec;
			meanTimesPrecision[0] = prec * mean;
		}
		protected void SetMeanAndPrecision(double mean, double precision)
		{
			Assert.IsTrue(Dimension == 1);
			...
			meanTimesPrecision[0] = precision * mean;
		}
#endif
		/// <summary>
		...
		{
			return GetMean(Vector.Zero(Dimension));
		}
		...
		public Evaluator<VectorGaussian, Vector> GetLogProbPrep()
		{
			LowerTriangularMatrix precL = new LowerTriangularMatrix(Dimension, Dimension);
			Vector iLb = Vector.Zero(Dimension);
			return delegate(VectorGaussian dist, Vector x)
			{
				return dist.GetLogProb(x, precL, iLb);
			};
		}
		...
		{
			LowerTriangularMatrix workspace = new LowerTriangularMatrix(Dimension, Dimension);
			return delegate(VectorGaussian dist, Vector result)
			{
				return dist.Sample(result, workspace);
			};
		}
		...
		/// The result may not be proper. No error is thrown in this case.
		/// </remarks>
		public void SetToRatio(VectorGaussian numerator, VectorGaussian denominator)
		{
			precision.SetToDifference(numerator.precision, denominator.precision);
			for (int i = 0; i < Dimension; i++) {
				if (Double.IsPositiveInfinity(numerator.precision[i, i])) {
					if (Double.IsPositiveInfinity(denominator.precision[i, i])) {
						if (numerator.Point[i] == denominator.Point[i]) {
				...
				} else if (Double.IsPositiveInfinity(denominator.precision[i, i])) {
					throw new DivideByZeroException();
				} else {
			...
			where T : CanGetMeanAndVariance<Vector, PositiveDefiniteMatrix>, CanSetMeanAndVariance<Vector, PositiveDefiniteMatrix>, SettableTo<T>, SettableToUniform
		{
			...
			// if dist1 == dist2 then we must return dist1, with no roundoff error
				...
				if (m1.Equals(m2)) { // catch this to avoid roundoff errors
		...
		/// </summary>
		/// <param name="that"></param>
		public VectorGaussian(VectorGaussian that) : this(that.Dimension)
		{
			SetTo(that);
		}
