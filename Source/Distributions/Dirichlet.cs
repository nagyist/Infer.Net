...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	/// <see cref="MicrosoftResearch.Infer.Maths.Sparsity.ApproximateWithTolerance(double)"/>.
	/// </para><para>
	...
	public class Dirichlet : IDistribution<Vector>,
		...
		SettableToRatio<Dirichlet>, SettableToPower<Dirichlet>, SettableToWeightedSum<Dirichlet>, CanGetLogAverageOf<Dirichlet>, CanGetLogAverageOfPower<Dirichlet>,
		...
		Sampleable<Vector>, CanGetMean<Vector>, CanGetVariance<Vector>, CanGetMeanAndVariance<Vector, Vector>, CanSetMeanAndVariance<Vector, Vector>
	{
		...
		public void SetMeanLog(Vector meanLog)
		{
			...
			PseudoCount.SetToFunction(PseudoCount, meanLog, (x, y) => Double.IsNegativeInfinity(y) ? 0.0 :
		...
		/// The result may not be proper, i.e. its parameters may be negative.
		/// </remarks>
		public void SetToRatio(Dirichlet numerator, Dirichlet denominator)
		{
		...
		/// <summary>
		/// Sets flag to allow an improper sum in <see cref="SetToSum"/>.
		/// </summary>
			...
			// if dist1 == dist2 then we must return dist1, with no roundoff error
				...
				if (dist1.IsPointMass) {
					if (dist2.IsPointMass) {
						if (!dist1.Point.Equals(dist2.Point)) throw new AllZeroException("dist1.Point = " + dist1.Point + Environment.NewLine + "dist2.Point = " + dist2.Point);
						Point = dist1.Point;
						return;
					} else {
					...
					if (weight1 > 0) {
						if (weight2 > 0) {
							...
							for (int i = 0; i < Dimension; i++) {
								bound = Math.Max(bound, minCount[i] / mean[i]);
							}
							boundViolated = (momentMatch.TotalCount < bound);
						} else {
							bound = Double.PositiveInfinity;
							for (int i = 0; i < Dimension; i++) {
								if (dist1.PseudoCount[i] < dist2.PseudoCount[i]) {
									bound = minCount[i] / mean[i];
								}
							}
							boundViolated = (momentMatch.TotalCount > bound);
						}
						...
						bound = Double.PositiveInfinity;
						for (int i = 0; i < Dimension; i++) {
							if (dist2.PseudoCount[i] < dist1.PseudoCount[i]) {
								bound = minCount[i] / mean[i];
							}
						}
						boundViolated = (momentMatch.TotalCount > bound);
		...
		/// </summary>
		/// <param name="dimension">Dimension</param>
		/// <param name="sparsity">The <see cref="Sparsity"/> specification. A specification of <see cref="MicrosoftResearch.Infer.Maths.Sparsity.ApproximateWithTolerance(double)"/>
		...
		/// A specification of <see cref="MicrosoftResearch.Infer.Maths.Sparsity.ApproximateWithTolerance(double)"/>
