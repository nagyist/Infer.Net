...
namespace MicrosoftResearch.Infer.Distributions
{
        ...
        CanGetAverageLog<ConjugateDirichlet>, CanGetLogNormalizer
    {
        ...
        public void SmartProposal(out double mean, out double variance)
        {
            ...
            {
                double expx = Math.Exp(x);
                double res = -mythis.Rate * expx + (mythis.K == 0.0 ? 0.0 : mythis.K * mythis.D * expx * (MMath.Digamma(mythis.D * expx) - MMath.Digamma(expx)) +
        ...
        public static double Newton1DMaximise(double initx, Converter<double,double> f, Converter<double,double> dfxfunc, Converter<double,double> dhxfunc)
        {
            ...
            do
            {
                ...
                while (f(xnew) < f(x))
                {
                    ...
                    xnew = x - step * dir;
                    if (step == 0.0)
                        throw new ApplicationException(errorMessage); // could return x here?
                }
        ...
        /// Approximation method to use for non-analytic expectations. 
        /// </summary>
		static public ApproximationMethod approximationMethod = ApproximationMethod.GaussHermiteQuadratureLaplace;
                ...
                switch (approximationMethod)
                {
                    ...
                    case ApproximationMethod.GaussHermiteQuadrature:
                        ...
                        for (int i = 0; i < nt; i++)
                        {
                            double x = nodes[i];
                            var expx = Math.Exp(x);
                            p[i] = Shape * x - Rate * expx + (K == 0.0 ? 0.0 : K * (MMath.GammaLn(D * expx) - D * MMath.GammaLn(expx))) - Gaussian.GetLogProb(x, proposalMean, proposalVariance);
                        ...
                        for (int i = 0; i < nt; i++)
                        {
                            ...
                            if (double.IsPositiveInfinity(z0))
                                throw new ApplicationException("Numerical problem in Conjugate Dirichlet Distribution (GetMeanAndVariance)");
                        }
                        break;
        ...
        public double GetMeanLog()
        {
            ...
            else
            {
                ...
                switch (approximationMethod)
                {
                    ...
                    case ApproximationMethod.GaussHermiteQuadrature:
                        ...
                        for (int i = 0; i < nt; i++)
                        {
                            ...
                            if (double.IsPositiveInfinity(z0))
                                throw new ApplicationException("inf");
                        }
                        break;
                ...
                if (double.IsNaN(res) || double.IsInfinity(res))
					throw new ApplicationException("Numerical problem in Conjugate Dirichlet Distribution (GetMeanLog)");
                return res;
            }
        }
        ...
        public double GetMeanLogGamma(double factor)
        {
            ...
            else
            {
                ...
                switch (approximationMethod)
                {
                    ...
                    case ApproximationMethod.GaussHermiteQuadrature:
                        ...
                        for (int i = 0; i < nt; i++)
                        {
                            double x = nodes[i];
                            var expx = Math.Exp(x);
                            p[i] = Shape * x - Rate * expx + (K == 0.0 ? 0.0 : K * (GammaLnExp(Math.Log(D) + x) - D * GammaLnExp(x))) - Gaussian.GetLogProb(x, proposalMean, proposalVariance);
                            p[i] += Math.Log(weights[i]);
                        }
                        maxP = p.Max();
                        for (int i = 0; i < nt; i++)
                ...
                double res = z1 / z0;
                if (double.IsNaN(res) || double.IsInfinity(res))
					throw new ApplicationException("Numerical problem in Conjugate Dirichlet Distribution (GetMeanLogGamma)");
				return res;
            }
        ...
        public double GetLogNormalizer()
        {
            if (IsProper())
            {
                ...
                if (double.IsNaN(res) || double.IsInfinity(res))
					throw new ApplicationException("Numerical problem in Conjugate Dirichlet Distribution (GetLogNormalizer)"); 
                return res;
            }
            else
            {
        ...
        public double GetAverageLog(ConjugateDirichlet that)
        {
            ...
            else
            {
                ...
                if (double.IsInfinity(result) || double.IsNaN(result))
					throw new ApplicationException("Numerical problem in Conjugate Dirichlet Distribution (GetAverageLog)"); 
                return result;
            }
        }
        ...
        /// <returns>The sample value</returns>
        [Stochastic]
        [MicrosoftResearch.Infer.Factors.ParameterNames("sample", "mean", "variance")]
        ...
        public void SetToProduct(ConjugateDirichlet a, ConjugateDirichlet b)
        {
            ...
            else
            {
                ...
                if (a.K == 0)
                    ...
                    throw new ApplicationException("Cannot multiple ConjugateDirichlet distributions with different D, i.e. you cannot have the same hyperparameter for two Dirichlet distributions with different dimensions");
        ...
        /// The result may not be proper. No error is thrown in this case.
        /// </remarks>
        public void SetToRatio(ConjugateDirichlet numerator, ConjugateDirichlet denominator)
        {
            ...
            else
            {
                ...
                if (numerator.K == 0)
                    ...
                    throw new ApplicationException("Cannot divide ConjugateDirichlet distributions with different D, i.e. you cannot have the same hyperparameter for two Dirichlet distributions with different dimensions");
        ...
        /// </remarks>
        public void SetToRatioProper(ConjugateDirichlet numerator, ConjugateDirichlet denominator)
        {
            if (numerator.IsPointMass)
            {
                if (denominator.IsPointMass)
