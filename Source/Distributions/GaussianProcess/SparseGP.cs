...
namespace MicrosoftResearch.Infer.Distributions
{
			...
			Sampleable<IFunction>
	{
		...
		{
			if (a.FixedParameters != b.FixedParameters) throw new ArgumentException("SparseGPs do not have the same FixedParameters.  a.FixedParameters = "+a.FixedParameters+", b.FixedParameters = "+b.FixedParameters);
		...
		public void SetToRatio(SparseGP numerator, SparseGP denominator)
		{
			if (numerator.FixedParameters != denominator.FixedParameters) throw new ArgumentException("SparseGPs do not have the same FixedParameters.  numerator.FixedParameters = "+numerator.FixedParameters+", denominator.FixedParameters = "+denominator.FixedParameters);
			...
			if (numerator.IsPointMass) {
				...
				throw new DivideByZeroException();
			} else {  // neither is point mass
				InducingDist.SetToRatio(numerator.InducingDist, denominator.InducingDist);
				pointFunc = null;
				ClearCachedValues();
			}
		}
		...
		public bool IsPointMass
		{
			get
			{
				return (pointFunc == null ? false : true);
			}
		}
		...
		public double MaxDiff(object thatd)
		{
          ...
          this.IsUniform() != that.IsUniform())
			...
			if (this.IsUniform() && that.IsUniform())
				return 0.0;
			if (this.IsPointMass) { // both point masses
		...
		public void SetToSum(double weight1, SparseGP value1, double weight2, SparseGP value2)
		{
			...
			if (value1.FixedParameters != value2.FixedParameters) throw new ArgumentException("SparseGPs do not have the same FixedParameters.  a.FixedParameters = "+value1.FixedParameters+", b.FixedParameters = "+value2.FixedParameters);
		...
		{
			if (this.FixedParameters != that.FixedParameters) throw new ArgumentException("SparseGPs do not have the same FixedParameters.  this.FixedParameters = "+this.FixedParameters+", that.FixedParameters = "+that.FixedParameters);
	...
	public class LinearSpline : IFunction
	{
		...
		public double KnotIncrem
		{
			...
			set { knotIncrem = value; knotIncremInv = 1.0 / knotIncrem; }
		}
