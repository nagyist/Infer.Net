...
namespace MicrosoftResearch.Infer.Distributions
{
		...
		CanGetAverageLog<WrappedGaussian>
	{
		...
		{
			if (a.Period < b.Period) { SetToProduct(b, a); return; }
			// a.Period >= b.Period
			if (a.IsUniform()) { SetTo(b); return; }
			if (b.IsUniform()) { SetTo(a); return; }
			if (a.IsPointMass) {
				if (b.IsPointMass && !a.Point.Equals(b.Point)) throw new AllZeroException();
				Point = a.Point;
				return;
			}
			if (b.IsPointMass) { Point = b.Point; return; }
			...
			if (kMax - kMin != intRatio-1) throw new ApplicationException("kMax - kMin != intRatio-1");
			...
			if (kMax - kMin > 100) throw new ApplicationException("kMax - kMin = "+(kMax-kMin));
...
#endif
			if (double.IsNaN(Gaussian.MeanTimesPrecision)) throw new ApplicationException("result is nan");
			Normalize();
		}
		/// <summary>
		...
		public void SetToRatio(WrappedGaussian numerator, WrappedGaussian denominator)
		{
