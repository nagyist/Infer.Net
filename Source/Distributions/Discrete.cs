...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	public class Discrete : IDistribution<int>, IXmlSerializable,
		SettableTo<Discrete>, SettableToProduct<Discrete>, Diffable, SettableToUniform,
		SettableToRatio<Discrete>, SettableToPower<Discrete>, SettableToWeightedSum<Discrete>, CanGetLogAverageOf<Discrete>, CanGetLogAverageOfPower<Discrete>,
		CanGetAverageLog<Discrete>, //CanGetLogNormalizer,
		Sampleable<int>, CanGetMean<double>, CanGetVariance<double>
	{
		...
		/// </summary>
		public int Dimension { get { return prob.Count; } }
		...
		public int Point
		{
			get
			{
				...
				else return (int)prob[1];
			}
			set
			{
				if (Dimension > 1) {
					prob.SetAllElementsTo(0.0); // must do this to ensure that the representation of a PointMass is unique
					prob[0] = Double.PositiveInfinity;
					prob[1] = value;
				}
			}
		}
		...
		{
			get { return (Dimension <= 1) || Double.IsPositiveInfinity(prob[0]); }
		}
		...
		{
			prob.SetAllElementsTo(1.0 / Dimension);
		}
		/// <summary>
		...
		{
			return prob.EqualsAll(1.0 / Dimension);
		}
		...
		public double GetLogAverageOf(Discrete that)
		{
			...
			} else if(that.Dimension == Dimension) {
		...
		{
			if (IsPointMass) {
				// save in case result == prob.
				int point = Point;
				result.SetAllElementsTo(0);
                if (result.Count>0) result[point] = 1;
			} else {
				result.SetTo(prob);
			}
			return result;
		...
		public void SetTo(Discrete value)
		{
			...
			} else { // value.Dimension > Dimension
				throw new ArgumentException("value.Dimension ("+value.Dimension+") > this.Dimension ("+Dimension+")");
		...
		{
			if (value < 0 || value >= numValues) throw new ArgumentException(String.Format("value ({0}) is not in the range [0, numValues-1 ({1})]", value, numValues));
		...
		{
			SerializableVector s = (SerializableVector)prob;
			s.WriteXml(writer);
		}
		#endregion
	}
