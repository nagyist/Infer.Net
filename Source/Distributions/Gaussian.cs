...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	public struct Gaussian : IDistribution<double>,
		...
		CanGetAverageLog<Gaussian>, CanGetLogNormalizer
	{
		/// <summary>
		...
		public void SetMeanAndPrecision(double mean, double precision)
		{
			if (Double.IsPositiveInfinity(precision)) {
				...
				Precision = precision;
				MeanTimesPrecision = Precision * mean;
			}
		}
		/// <summary>
...
#if false
		// these should not be properties since they are not fast.
		public double Mean { get { return GetMean(); } }
		public double Variance { get { return GetVariance(); } }
#endif
		/// <summary>
		...
		public double GetLogProb(double x)
		{
			if (IsPointMass) {
				...
				// TM: this method is most efficient since it involves a single division.
		...
		/// The result may not be proper. No error is thrown in this case.
		/// </remarks>
		public void SetToRatio(Gaussian numerator, Gaussian denominator)
		{
			...
			where T : CanGetMeanAndVarianceOut<double, double>, CanSetMeanAndVariance<double, double>, SettableToUniform, SettableTo<T>
		{
			if (weight1 + weight2 == 0) result.SetToUniform();
				...
				if (m1 == m2) {  // catch this to avoid roundoff errors
		...
		{
			if (IsPointMass) {
				return "Gaussian.PointMass(" + Point.ToString("g4") + ")";
			} else if (IsUniform()) {
				return "Gaussian.Uniform";
			} else if (IsProper()) {
				double mean, variance;
				GetMeanAndVariance(out mean, out variance);
				return "Gaussian(" + mean.ToString("g4") + ", " + variance.ToString("g4") + ")";
			} else {
				return "Gaussian(m/v=" + MeanTimesPrecision.ToString("g4") + ", 1/v=" + Precision.ToString("g4") + ")";
			}
		}
	}
}
