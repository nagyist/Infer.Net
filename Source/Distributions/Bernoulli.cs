...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	public struct Bernoulli : IDistribution<bool>,
		...
		SettableToRatio<Bernoulli>, SettableToPower<Bernoulli>, SettableToWeightedSum<Bernoulli>,
		...
		CanGetAverageLog<Bernoulli>, CanGetLogNormalizer
	{
		/// <summary>
		/// Log odds parameter
		/// </summary>
		...
		{
			if (probFalse < 0 || probFalse > 1) throw new ArgumentException(String.Format("probFalse = {0} is not in [0,1]", probFalse), "probFalse");
		...
		{
			if (logProbTrue > 0) throw new ArgumentException(String.Format("logProbTrue = {0} is not <= 0", logProbTrue), "logProbTrue");
		...
		{
			if (logProbFalse > 0) throw new ArgumentException(String.Format("logProbFalse = {0} is not <= 0", logProbFalse), "logProbFalse");
		...
		public bool IsPointMass
		{
			get
			{
				return Double.IsInfinity(LogOdds);
			}
		}
		...
		public bool Point
		{
			get
			{
				return Double.IsPositiveInfinity(LogOdds);
			}
			set
			{
				LogOdds = value ? Double.PositiveInfinity : Double.NegativeInfinity;
			}
		}
...
#pragma warning restore 1591
		...
		public void SetToRatio(Bernoulli numerator, Bernoulli denominator)
		{
		...
		public static double LogitProbEqual(double x, double y)
		{
			...
			double sum = x + y, diff = Math.Abs(x - y);
			if (x == y) diff = 0;  // in case x and y are Inf
			//double min = 0.5 * (sum - diff);  // same as Math.Min(x,y)
			double min = Math.Min(x, y);
			double result = min + Math.Log((1 + Math.Exp(-sum)) / (1 + Math.Exp(-diff)));
			return result * sign;
		}
		/// <summary>
		...
		{
			if (x >= 0 && y >= 0) {
				return x + y + Math.Log(1 + Math.Exp(-x) + Math.Exp(-y));
			} else if (x == y) {
				// avoid subtracting infinities
				return x + Math.Log(2 + Math.Exp(x));
			} else if (x > y) {
				// y must be negative
				return x + Math.Log(1 + Math.Exp(y - x) + Math.Exp(y));
			} else {
				// x must be negative
				return y + Math.Log(1 + Math.Exp(x - y) + Math.Exp(x));
			}
		}
