...
namespace MicrosoftResearch.Infer.Distributions
{
			...
			CanGetAverageLog<TruncatedGaussian>
	{
		...
		{
			if (!(thatd is TruncatedGaussian)) return Double.PositiveInfinity;
		...
		public void SetToProduct(TruncatedGaussian a, TruncatedGaussian b)
		{
			...
			if (LowerBound > UpperBound) throw new AllZeroException();
		...
		{
			if (IsPointMass) {
				if (that.IsPointMass && this.Point == that.Point) return 0.0;
				else return that.GetLogProb(this.Point);
