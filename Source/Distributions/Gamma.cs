...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	public struct Gamma : IDistribution<double>, //ICursor,
		...
		CanGetAverageLog<Gamma>, CanGetLogNormalizer
	{
		/// <summary>
		...
		{
			double result = (shape - 1) * Math.Log(x) - x * rate;
		...
		/// The result may not be proper. No error is thrown in this case.
		/// </remarks>
		public void SetToRatio(Gamma numerator, Gamma denominator)
		{
		...
		/// </remarks>
		public void SetToRatioProper(Gamma numerator, Gamma denominator)
		{
			if (numerator.IsPointMass) {
				if (denominator.IsPointMass) {
					if (numerator.Point.Equals(denominator.Point)) {
				...
				if (Shape < 0 || Rate < 0) {
					...
					Shape = mean * denominator.Rate - denominator.Shape + 1;
					if (Shape < 0) {
						Shape = 0;
						Rate = (denominator.Shape - 1) / mean - denominator.Rate;
					} else {
						Rate = 0;
					}
				}
		...
		public double MaxDiff(object thatd)
		{
			...
			return Math.Max(MMath.AbsDiff(Rate, that.Rate), MMath.AbsDiff(Shape, that.Shape));
		}
		/// <summary>
