...
namespace MicrosoftResearch.Infer.Distributions
{
		...
		CanGetAverageLog<GammaPower>, CanGetLogNormalizer
	{
		...
		/// <returns>The sample value</returns>
		[Stochastic]
		[MicrosoftResearch.Infer.Factors.ParameterNames("sample", "mean", "variance", "power")]
		...
		/// The result may not be proper. No error is thrown in this case.
		/// </remarks>
		public void SetToRatio(GammaPower numerator, GammaPower denominator)
		{
			...
			if (numerator.IsPointMass) {
				...
				Shape = numerator.Shape - denominator.Shape + numerator.Power;
