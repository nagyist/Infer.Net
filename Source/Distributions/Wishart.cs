...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	public class Wishart : IDistribution<PositiveDefiniteMatrix>, 
		...
		SettableToWeightedSum<Wishart>, CanGetAverageLog<Wishart>, CanGetLogNormalizer
	{
		...
		{
			if (IsPointMass) return Point.LogDeterminant();
			// E[logdet(X)] = -logdet(B) + sum_{i=0..d-1} digamma(a)
			double s = 0;
			int d = Dimension;
			for (int i = 0; i < d; i++) s += MMath.Digamma(Shape - i * 0.5);
		...
		public static double GetLogNormalizer(double shape, PositiveDefiniteMatrix rate)
		{
			...
			if(!(shape > 0.5 * (dimension - 1))) return 0.0;
		...
		/// <returns>The sample</returns>
		[Stochastic]
		[MicrosoftResearch.Infer.Factors.ParameterNames("sample", "shape", "scale")]
		...
		/// The result may not be proper. No error is thrown in this case.
		/// </remarks>
		public void SetToRatio(Wishart numerator, Wishart denominator)
		{
			if (numerator.IsPointMass) {
				...
				shape = numerator.shape - denominator.shape + 0.5 * (Dimension + 1);
	...
	where T : CanGetMeanAndVariance<PositiveDefiniteMatrix, PositiveDefiniteMatrix>, CanSetMeanAndVariance<PositiveDefiniteMatrix, PositiveDefiniteMatrix>, SettableToUniform, SettableTo<T>
		...
		{
			get { return rate.SourceArray; }
			set { /*rate.SourceArray = value; shape.SourceArray = value;*/ }
		}
        ...
        public override string ToString()
		{
			if (IsPointMass) {
				...
				return StringUtil.JoinColumns("Wishart(", shape, ", ", rate.Inverse(), ")");
			} else {
				return StringUtil.JoinColumns("Wishart(", shape, ", rate=", rate, ")");
			}
		}
	}
}
