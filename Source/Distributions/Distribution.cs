...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	public interface CanGetMean<MeanType>
	{
		/// <remarks><para>
		/// </para><para>
	...
	/// </summary>
	/// <typeparam name="MeanType">Type of the mean value</typeparam>
	public interface CanSetMean<MeanType>
	{
		/// <summary>
	...
	public interface CanGetVariance<VarType>
	{
		/// <summary>
	...
	public interface CanSetMeanAndVariance<MeanType, VarType>
	{
		/// <summary>
	...
	public interface CanGetLogAverageOfPower<T>
	{
		/// <summary>
	...
	/// </summary>
	/// <typeparam name="T">The distribution type</typeparam>
	public interface CanGetAverageLog<T>
	{
		/// <summary>
	...
	public static class Distribution
	{
		...
		public static QualityBand GetQualityBand(Type type)
		{
			if (!HasDistributionType(type))
				throw new ApplicationException("Trying to get a quality band for a non-distribution type" + type.Name);
			...
			if (type.IsGenericType)
			{
				...
				foreach (Type gt in gtypes)
				{
					if (HasDistributionType(gt))
					{
						QualityBand qb1 = GetQualityBand(gt);
						if ((!found ) || qb1 < qb)
							qb = qb1;
						found = true;
					}
			...
			{
				var qb1 = Quality.GetQualityBand(type);
				if ((!found) || qb1 < qb)
					qb = qb1;
			}
			return qb;
		...
		public static ArrayType ToArray<ArrayType>(object distributionArray)
		{
			...
			MethodInfo method = new Func<object, ArrayType>(Distribution.ToArray<object, ArrayType>).Method.GetGenericMethodDefinition();
			method = method.MakeGenericMethod(distributionType, typeof(ArrayType));
			return (ArrayType)method.Invoke(null, new object[] { distributionArray });
		}
		/// <summary>
		/// Convert a distribution over an array variable to an array of element distributions.
		/// </summary>
		...
		internal static Converter<TInput, TOutput> GetArrayConverter<TInput, TOutput>()
		{
			...
			if (!outputElementType.IsArray) {
				...
				if(method == null) {
					...
					throw new ArgumentException("Cannot convert from "+inputName+" to "+StringUtil.TypeToString(typeof(TOutput))+": "+message);
				}
				...
				MethodInfo thisMethod = new Func<Converter<object, object>>(Distribution.GetArrayConverter<object, object>).Method.GetGenericMethodDefinition();
				thisMethod = thisMethod.MakeGenericMethod(elementType, outputElementType);
				object itemConverter = thisMethod.Invoke(null, new object[0]);
				...
				foreach (MethodInfo sm in staticMeths) {
					if (sm.Name == "Array") {
						...
						if (mi.GetParameters()[0].ParameterType == arrType) {
							result = mi.Invoke(null, new object[] { arrayOfDistributions });
							break;
						}
					}
				}
 ...
 where T : SettableToProduct<U>, SettableTo<U>, U, SettableToUniform
		...
		public static void SetToRatio<T1, T2>(T1[] result, T2[] numerator, T2[] denominator)
	...
	public static class Distribution<T>
	{
			...
			where Distribution : IDistribution<T>
		{
			...
			return (IDistribution<T[][]>)method.Invoke(null, new object[] { array });
		}
			...
			where Distribution : IDistribution<T>
		{
			...
			return (IDistribution<T[,][]>)Activator.CreateInstance(arrayType, array.GetLength(0), array.GetLength(1), new Func<int, int, TItem>(
			...
			where Distribution : IDistribution<T>
		{
			...
			MethodInfo method = new Func<Distribution[][][], IDistribution<T[][][]>>(Distribution<T>.Array111<Distribution, object, object>).Method.GetGenericMethodDefinition();
			...
			where Distribution : IDistribution<T>
		{
			...
			return (IDistribution<T[][][]>)Activator.CreateInstance(arrayType, array.Length, new Func<int, TMiddle>(
					delegate(int i) {
						return (TMiddle)Activator.CreateInstance(typeof(TMiddle), array[i].Length, new Func<int, TInner>(
							delegate(int j) {
