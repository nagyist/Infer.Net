...
namespace MicrosoftResearch.Infer.Distributions
{
				...
				Sampleable<double>, CanGetMean<double>, CanGetVariance<double>, CanGetMeanAndVarianceOut<double, double>, CanSetMeanAndVariance<double, double>
	{
		...
		{
			if (IsPointMass) {
				eLogP = Math.Log(Point);
				eLogOneMinusP = Math.Log(1 - Point);
			} else if (!IsProper()) {
				throw new ImproperDistributionException(this);
			} else {
		...
		public double MaxDiff(object thatd)
		{
			...
			return Math.Max(MMath.AbsDiff(that.TrueCount, TrueCount),
		...
		public double GetLogProb(double value)
		{
			if (IsPointMass) {
				...
				if (TrueCount != 1) p += (TrueCount - 1) * Math.Log(value);
				if (FalseCount != 1) p += (FalseCount - 1) * Math.Log(1 - value);
		...
		/// No error is thrown in this case.
		/// </remarks>
		public void SetToRatio(Beta numerator, Beta denominator)
		{
