...
{
	/// <summary>
	/// Provides outgoing messages for <see cref="Factor.GaussianFromMeanAndVariance"/>, given random arguments to the function.
	/// </summary>
	...
	public static class GaussianFromMeanAndVarianceOp
	{
		...
		[Skip]
		public static double LogEvidenceRatio(Gaussian sample, Gaussian mean, double variance)		{			return 0.0;		}
		[Skip]
		public static double LogEvidenceRatio(Gaussian sample, double mean, double variance) { return 0.0; }
		...
		{
			if (variance.IsPointMass) return LogAverageFactor(sample, mean, variance.Point);
			double mx,vx,mm,vm;
			double a,b;
		...
		/// </para></remarks>
		/// <exception cref="ImproperMessageException"><paramref name="variance"/> is not a proper distribution</exception>
		[Quality(QualityBand.Experimental)]
		public static Gaussian SampleAverageConditional(Gaussian sample, double mean, [Proper] Gamma variance)
		{
		...
		public static void VarianceGammaTimesGaussianMoments5(double a, double m, double v, out double mu, out double vu)
		{
			...
			mu = Z1/Z0;
			vu = Z2/Z0 - mu*mu;
			if (Double.IsNaN(vu)) throw new Exception("result is NaN");
			if(vu < 0) throw new Exception("vu < 0");
		}
		/// <summary>
		...
		{
			if (a < 1) throw new ArgumentException("a < 1", "a");
			if (a == 1) {
				double sqrtV = Math.Sqrt(v);
		...
		public static double[][] NormalVGMomentTable(int nMax, int aMax, double m, double v, double[] moments1)
		{
			...
			if (aMax < 1) throw new ArgumentException("aMax < 1", "aMax");
			// if (a == 1) {
			moments[0] = moments1;
			if (aMax >= 2) {
		...
		/// <exception cref="ImproperMessageException"><paramref name="variance"/> is not a proper distribution</exception>
		[Quality(QualityBand.Experimental)]
		public static Gaussian MeanAverageConditional([SkipIfUniform] Gaussian sample, Gaussian mean, [Proper] Gamma variance)
		{
			return SampleAverageConditional(mean, sample, variance);
		}
		...
		public static Gamma VarianceAverageConditional([SkipIfUniform] Gaussian sample, [SkipIfUniform] Gaussian mean, [Proper] Gamma variance)
		{
			...
			Gamma result = Gamma.FromMeanAndVariance(vmp, vvp)/variance;
			if(result.Shape < 1) {
				// force result.Shape = 1, choose Rate to match mean only
				result = Gamma.FromShapeAndRate(variance.Shape, variance.Shape/vmp)/variance;
			}
			return result;
		}
